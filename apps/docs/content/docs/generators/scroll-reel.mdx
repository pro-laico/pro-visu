...
| `durationMs` | Clip length in ms (when not using `choreography`/`autoSections`/`routes`). Default `6000`. |
| `deviceScaleFactor` | Render scale — higher captures crisper, then downscales into the video. Default `2` (max `4`). |
| `easing` | Scroll easing: `linear`, `ease-in-out-cubic` (default), `ease-in-out-quad`, `ease-out-cubic`, `ease-in-out-sine`, `ease-in-out-expo`, `ease-out-quint`. |
...
| `workers` | Parallel render contexts for `"frames"` (auto-picks from cores + free memory; all captures share a run-wide context budget). |
