...
| Option | Type | Default | Meaning |
|---|---|---|---|
| `holdMs` | `number` | `2000` | How long each colour stays fully open (ms). |
| `transitionMs` | `number` | `700` | Crossfade length between colours (ms). |
...
| `durationMs` | `number` | derived | Override the derived clean-loop length (ms). |

### Layout & styling

| Option | Type | Default | Meaning |
|---|---|---|---|
| `grownFlex` | `number` (≥1) | `12` | How many times a sliver's share a fully-open band takes. |
