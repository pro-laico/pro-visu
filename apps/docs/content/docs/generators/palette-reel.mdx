...
| `bounce` | `boolean` | `true` | Ping-pong the sweep so every handoff is between neighbours (avoids a last→first pinch at the loop seam). |
...
| Option | Type | Default | Meaning |
|---|---|---|---|
| `grownFlex` | `number` | `12` | How many slivers' share a fully-open band takes. |
...
| `fontFile` | `string` | system bold sans | Custom font served into the render. |
