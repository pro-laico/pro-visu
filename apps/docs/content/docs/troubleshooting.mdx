...
## "JavaScript heap out of memory" / run stops with a low-memory warning

Heavy frame-stepped jobs (large walls especially) can exceed Node's default heap. Raise it with
`settings.maxMemoryMB` (the CLI re-execs itself with a bigger heap) or lower
`settings.concurrency`. The run watches its own memory and stops gracefully before a crash —
already-finished assets are kept, and are skipped on the next run when `--cache` is on.
...
`pro-visu reset` tears down orphaned server/browser process trees and temp dirs recorded by the
interrupted run. Add `--force` if the run still looks active but is actually stuck.
