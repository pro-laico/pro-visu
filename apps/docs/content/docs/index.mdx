...
> Captures run in a managed Chromium and encode with a managed ffmpeg — both downloaded once on
> first use and shared across projects, so no global installs are required.
