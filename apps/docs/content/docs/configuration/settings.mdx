...
`capture` flips a switch the site itself honours. (It's also distinct from scroll-reel's
[capture *strategy*](/docs/generators/scroll-reel#capture-strategy) — `"frames"` vs `"realtime"` —
which controls how frames are recorded, not what the site renders.)
