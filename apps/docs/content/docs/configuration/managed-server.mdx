...
Only `command` is required. **Reference** is the interactive view; **TypeScript** is the same
shape in code, every field at its default.
...
settings: {
  server: {
    command: "next start",              // required; PORT/HOST injected
    // build: "next build",             // optional one-shot build first
    // url: "http://127.0.0.1:3101",    // defaults to http://127.0.0.1:<port>
    port: 3101,
    // cwd: ".",                        // defaults to the config dir
