...
A sample target website + harness for **auto-showcase**: it's both the site the generators
...
- `app/gallery/page.tsx` — reads `public/showcase/manifest.json` to preview generated assets.
...
The site keeps stable selectors so `showcase.config.ts` can choreograph interactions:
