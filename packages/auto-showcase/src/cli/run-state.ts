...
/**
 * A small record of what a `showcase generate` run spawned, written to `<outDir>/.showcase-run.json`
 * and deleted on a clean exit. If the run is killed hard (Ctrl+C / crash), the file survives so
 * `showcase reset` can find and tear down the orphaned server + browser + temp dirs.
 */
export interface RunState {
  /** The `showcase generate` process id (used to tell "still running" from "orphaned"). */
  pid: number;
  startedAt: string;
