...
/**
 * Build a draft-07 JSON Schema for a `showcase.config.json`, assembled from the live, registered
 ...
 * from the registry at runtime, `showcase init --json` / `showcase schema` materialize a schema that
...
export function generateConfigJsonSchema(): JsonObject {
  ...
  return {
    ...
    title: "auto-showcase config",
    description: "Configuration for the auto-showcase CLI (showcase.config.json).",
    type: "object",
    required: ["assets"],
    additionalProperties: false,
    properties: {
