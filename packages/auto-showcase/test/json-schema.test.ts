...
describe("runSchema", () => {
  ...
  it("writes a valid showcase.schema.json to the cwd", async () => {
    await runSchema({ cwd: dir });
    const written = JSON.parse(readFileSync(path.join(dir, "showcase.schema.json"), "utf8"));
    expect(written.title).toBe("auto-showcase config");
    expect(written.required).toContain("assets");
  });

  it("honors a custom --out path", async () => {
    await runSchema({ cwd: dir, out: "config/showcase.schema.json" });
    const written = JSON.parse(
      readFileSync(path.join(dir, "config", "showcase.schema.json"), "utf8"),
    );
