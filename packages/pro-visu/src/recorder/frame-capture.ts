...
function acquireContextSlot(): { wait: Promise<void> | null; release: () => void } {
  ...
  return { wait: budget.tryAcquire() ? null : budget.acquire(), release: () => budget.release() };
}
...
async function renderChunk<S>(a: ChunkArgs<S>): Promise<void> {
  ...
  try {
    ...
    const context = await a.browser.newContext({ viewport: { width: a.width, height: a.height }, deviceScaleFactor: a.deviceScaleFactor });
    ...
    try {
      ...
      let finished = false;
      try {
        const shotOptions = a.frameFormat === "png" ? ({ type: "png" } as const) : ({ type: "jpeg", quality: a.jpegQuality } as const);
...
export async function captureFramedVideo<S>(args: FrameStepArgs<S>): Promise<void> {
  ...
  const encoderThreads = ranges.length > 1 ? Math.max(2, Math.floor((os.cpus()?.length ?? 2) / ranges.length)) : undefined;
  let completed = 0;
  ...
  const segs = ranges.map((r, i) => ({ ...r, seg: path.join(args.tmpDir, `seg-${i}-${path.basename(args.outPath)}`) }));
  ...
  try {
    await Promise.all(
      segs.map((r) =>
        renderChunk({ ...common, frameStart: r.start, frameEnd: r.end, outPath: r.seg, signal: chunkAbort.signal }).catch((err) => {
          chunkAbort.abort();
          throw err;
        }),
      ),
  ...
  await concatMp4(segs.map((r) => r.seg), args.outPath, args.logger, args.signal);
}
